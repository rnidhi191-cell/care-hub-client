...
export default function ReviewCycles() {
  ...
  const deleteCycle = async (cycle) => {
    const result = await Swal.fire({ title: 'Delete this review cycle?', text: `This permanently deletes ${cycle.title} and every linked self, colleague, and manager review.`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc2626', confirmButtonText: 'Yes, delete cycle' });
    if (!result.isConfirmed) return;
    try {
      await api.delete(`/review-cycles/${cycle._id}`);
      await Swal.fire({ icon: 'success', title: 'Cycle deleted', timer: 1200, showConfirmButton: false });
      fetchData();
    } catch (err) { Swal.fire({ icon: 'error', title: 'Could not delete cycle', text: err.response?.data?.message || 'Please try again.' }); }
  };
